...
- **`best_take.py <video> --script <txt>`** — script-driven take selection. Picks the **single** main-speaker take whose token sequence is closest (Levenshtein) to the script and emits a cut MP4. Auto-detects main speaker by per-speaker RMS. No approval gate. See `videos/edit/benchmarks.md` workflow #2.
- **`level_audio.py <media> [--dialogue|--music|--peak-min X --peak-max Y]`** — peak-normalize audio to a dBFS target window. Linear gain (never drops levels below the target). Presets: `--dialogue` = [-6, -3] dBFS, `--music` = [-24, -20] dBFS. Verifies the output peak post-encode and logs to `videos/edit/level_log.jsonl`. Use when a source is already synced and just needs leveling.
- **`sync_audio.py` and `match_pairs.py` accept `--level-dialogue` / `--level-music` / `--target-peak-db MIN MAX`** to bake leveling into the sync encode (no extra pass — the volume filter is injected into the existing chain). The auto-pair prompt enables `--level-dialogue` by default.
